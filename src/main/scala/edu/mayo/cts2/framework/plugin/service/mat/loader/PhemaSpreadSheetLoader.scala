...
@Component
class PhemaSpreadSheetLoader extends Loader {
  ...
  def VALUE_SET_VALUE_SET_NAME_CELL = 1
  def VALUE_SET_VALUE_SET_DEFINITION_CELL = 2
  def VALUE_SET_CODE_SYSTEM_CELL = 3
  def VALUE_SET_CODE_SYSTEM_VERSION_CELL = 4
  def VALUE_SET_CONCEPT_CELL = 5
  def VALUE_SET_DESCRIPTION_CELL = 6
  ...
  Map[String, DefinitionResult] = {
    var context = new EntryContext(null, null, null, null, null)
...
//                  valueSet.addVersion(version)
  ...
  def createValueSet(vsr: ValueSetRow, resources: ResourcesResult) = {
    ...
    vs.setFormalName(vsr.context.valueSetFormalName)
    vs.setHref(res href)
    vs.setUri(res uri)
    vs
  }
  ...
  def rowToValueSetRow(row: Row, context: EntryContext): Option[ValueSetRow] = {
    ...
    getCellValue(row.getCell(VALUE_SET_CONCEPT_CELL)).flatMap(code => {
      val newContext = new EntryContext(context.codeSystem, context.codeSystemVersion, context.valueSetName, context.valueSetFormalName, context.valueSetDefinition)
      getCellValue(row.getCell(VALUE_SET_VALUE_SET_CELL)).foreach(newContext.valueSetName = _)
      getCellValue(row.getCell(VALUE_SET_VALUE_SET_NAME_CELL)).foreach(newContext.valueSetFormalName = _)
